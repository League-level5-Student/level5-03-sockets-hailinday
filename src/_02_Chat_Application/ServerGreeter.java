...
public class ServerGreeter extends Thread {
	...
	public void run() {
		...
		while (runs) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Client waiting to connect");
				Socket sockObj = servSock.accept();
				JOptionPane.showMessageDialog(null, "Client connected");
				DataInputStream inStreamObj = new DataInputStream(sockObj.getInputStream());
				DataOutputStream outStreamObj = new DataOutputStream(sockObj.getOutputStream());
				String input = JOptionPane.showInputDialog("Please enter a message to client: ");
				outStreamObj.writeUTF(input);
				System.out.println(inStreamObj.readUTF());
				sockObj.close();
	...
	public static void main(String[] args) {
		...
		Thread serverThread;
		try {
			serverThread = new Thread(new ServerGreeter());
			serverThread.start();
		} catch (IOException e) {
